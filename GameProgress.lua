...
local StoryMissions = 7^2       -- 7 levels * 7 nissions
local AllMissions = 7^2 + 7*5   -- 7 levels * 7 missions + 7 levels * 5 bonus missions
  ...
  -- Checking if mission has been completed before
    if missions[level][mission] == false then  
        ...
        if mission ~= 'BM' and mission ~= 'WG' and not string.find(mission, 'SR') and mission ~= 'M0' then
            CompletedMissions.StoryMissions = CompletedMissions.StoryMissions + 1
       end
    end
    ...
    CompletedMissions.StoryProgress = (CompletedMissions.StoryMissions*100)/StoryMissions
